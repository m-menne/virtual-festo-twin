...
volatile uint32_t *MAXIM_CS = MAXIM_CS_Adr;
volatile uint32_t *MAXIM_RD = MAXIM_RD_Adr;
volatile uint32_t *MAXIM_SHDN = MAXIM_SHDN_Adr;
volatile uint32_t *MAXIM_CONVST = MAXIM_CONVST_Adr;
volatile uint32_t *MAXIM_WR = MAXIM_WR_Adr;
volatile uint32_t *MAXIM_EOC = MAXIM_EOC_Adr;
volatile int *MAXIM_DATA = MAXIM_DATA_Adr;

// Function to initialize the analog_in board
void init_analog_inputs() {
	...
	*MAXIM_DATA = 0x8;
	...
	// Some waiting cycles
	for (volatile uint32_t ctr = 10000; ctr > 0; --ctr);
